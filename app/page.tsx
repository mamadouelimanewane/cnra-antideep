...
const FEATURES = [
  { icon: "🔍", title: "Détection de deepfakes", desc: "Algorithmes d'IA de pointe pour identifier les vidéos et audios falsifiés diffusés sur les médias." },
  { icon: "📡", title: "Veille désinformation", desc: "Surveillance continue des campagnes de désinformation ciblant le paysage audiovisuel sénégalais." },
  { icon: "🗄️", title: "Base de signatures IA", desc: "Référentiel des signatures numériques de contenus synthétiques connus et répertoriés." },
  { icon: "⚠️", title: "Alertes & signalements", desc: "Système d'alerte multi-niveaux pour une réponse rapide face aux menaces informationnelles." },
]
...
export default function LandingPage() {
  ...
  return (
    <>
      ...
      `}</style>
      <div style={{ minHeight:"100vh", background:"#09060f", color:"#fff", fontFamily:"system-ui,sans-serif", position:"relative", overflow:"hidden" }}>

        <div style={{ position:"fixed", inset:0, pointerEvents:"none",
          background:`radial-gradient(ellipse 80% 60% at 50% -10%, rgba(168,85,247,0.12), transparent)` }} />
        <div style={{ position:"fixed", inset:0, pointerEvents:"none",
        ...
        <header style={{ position:"sticky", top:0, zIndex:50, borderBottom:"1px solid rgba(255,255,255,0.06)",
          background:"rgba(9,6,15,0.88)", backdropFilter:"blur(20px)" }}>
          <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 2rem", height:60, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:32, height:32, borderRadius:8, background:"linear-gradient(135deg,#4a1072,#9333ea)",
                border:`1px solid rgba(168,85,247,0.4)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>🛡️</div>
              <div>
                <p style={{ fontSize:13, fontWeight:700, color:"#fff", margin:0 }}>AntiDeep</p>
                <p style={{ fontSize:9, color:COLOR, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", margin:0 }}>CNRA · Détection IA · Deepfakes</p>
              </div>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <div className="pulse" style={{ width:6, height:6, borderRadius:"50%", background:"#10b981" }} />
              <span style={{ fontSize:11, color:"#10b981", fontWeight:600 }}>IA ACTIVE</span>
            </div>
          </div>
        </header>
        ...
        <section style={{ maxWidth:1100, margin:"0 auto", padding:"5rem 2rem 4rem", textAlign:"center" }}>
          ...
          <div className="au2">
            <h1 style={{ fontSize:"clamp(2.2rem,5vw,4rem)", fontWeight:900, lineHeight:1.05, letterSpacing:"-0.03em", margin:"0 0 1.25rem" }}>
              ...
              <span style={{ background:`linear-gradient(135deg,#fff 0%,#e9d5ff 50%,${COLOR} 100%)`,
          ...
          <p className="au3" style={{ fontSize:"clamp(1rem,2vw,1.2rem)", color:"rgba(255,255,255,0.5)",
            ...
            La sentinelle numérique du CNRA contre les <strong style={{ color:"rgba(255,255,255,0.85)" }}>deepfakes et la désinformation</strong> —
            détectez, signalez et neutralisez les contenus falsifiés qui menacent l&apos;information sénégalaise.
          </p>

          <div className="au4" style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
            <Link href="/dashboard" style={{
              display:"inline-flex", alignItems:"center", gap:8,
              background:`linear-gradient(135deg,#4a1072,${COLOR})`,
            ...
            {FEATURES.map((f, i) => (
              <div key={i} className={`card fc fc${i+1}`}
                ...
                style={{
                  ...
                  border: `1px solid ${hovered===i ? "rgba(168,85,247,0.3)" : "rgba(255,255,255,0.07)"}`,
        ...
        <footer style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:"1.5rem 2rem", textAlign:"center" }}>
          <p style={{ fontSize:11, color:"rgba(255,255,255,0.2)", margin:0 }}>
            © 2025 CNRA · AntiDeep · Développé par <span style={{ color:"rgba(255,255,255,0.4)", fontWeight:600 }}>Processingenierie</span>
          </p>
        </footer>
      </div>
    </>
